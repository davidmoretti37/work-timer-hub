  ...
  useEffect(() => {
    ...
    class AutoDriver {
      ...
      constructor(mouse: MouseClass, manager: any, opts: any) {
        this.mouse = mouse; this.manager = manager; this.enabled = opts.enabled; this.speed = opts.speed; this.resumeDelay = opts.resumeDelay || 3000; this.rampDurationMs = (opts.rampDuration || 0) * 1000; this.pickNewTarget();
      }
      ...
      update() {
        if (!this.enabled) return; const now = performance.now(); const idle = now - this.manager.lastUserInteraction; if (idle < this.resumeDelay) { if (this.active) this.forceStop(); return; }
...
`;
    ...
    class ShaderPass {
      props: any; uniforms: any; scene: THREE.Scene | null = null; camera: THREE.Camera | null = null; material: any; geometry: any; plane: any; line: any;
      ...
      update() { (Common.renderer as THREE.WebGLRenderer).setRenderTarget(this.props.output || null); (Common.renderer as THREE.WebGLRenderer).render(this.scene as any, this.camera as any); (Common.renderer as THREE.WebGLRenderer).setRenderTarget(null); }
    }
    ...
    class Poisson extends ShaderPass { constructor(simProps: any) { super({ material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: simProps.dst_.texture }, divergence: { value: simProps.src.texture }, px: { value: simProps.cellScale } } }, output: simProps.dst, output0: simProps.dst_, output1: simProps.dst }); this.init(); }
      update({ iterations }: any) { let p_in: any, p_out: any; for (let i = 0; i < iterations; i++) { if (i % 2 === 0) { p_in = this.props.output0; p_out = this.props.output1; } else { p_in = this.props.output1; p_out = this.props.output0; } this.uniforms.pressure.value = p_in.texture; this.props.output = p_out; super.update(); } return p_out; }
    }
    ...
    class Simulation {
      ...
      createAllFBO() { const type = this.getFloatType(); const opts: any = { type, depthBuffer: false, stencilBuffer: false, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, wrapS: THREE.ClampToEdgeWrapping, wrapT: THREE.ClampToEdgeWrapping }; for (let key in this.fbos) { this.fbos[key] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts); } }
      createShaderPass() {
    ...
    class Output { simulation: any; scene: THREE.Scene | null = null; camera: THREE.Camera | null = null; output: any;
      constructor() { this.init(); }
      init() { this.simulation = new Simulation(); this.scene = new THREE.Scene(); this.camera = new THREE.Camera(); this.output = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), new THREE.RawShaderMaterial({ vertexShader: face_vert, fragmentShader: color_frag, transparent: true, depthWrite: false, uniforms: { velocity: { value: this.simulation.fbos.vel_0.texture }, boundarySpace: { value: new THREE.Vector2() }, palette: { value: paletteTex }, bgColor: { value: bgVec4 } } })); (this.scene as THREE.Scene).add(this.output); }
      addScene(mesh: any) { (this.scene as THREE.Scene).add(mesh); }
      resize() { this.simulation.resize(); }
      render() { (Common.renderer as THREE.WebGLRenderer).setRenderTarget(null); (Common.renderer as THREE.WebGLRenderer).render(this.scene as any, this.camera as any); }
      update() { this.simulation.update(); this.render(); }
    }

    class WebGLManager {
      props: any; lastUserInteraction = performance.now(); _loop: any; _resize: any; _onVisibility: any; running = false; output: any; autoDriver: any;
      constructor(props: any) { this.props = props; Common.init(props.$wrapper); Mouse.init(props.$wrapper); Mouse.autoIntensity = props.autoIntensity; Mouse.takeoverDuration = props.takeoverDuration; this.lastUserInteraction = performance.now(); Mouse.onInteract = () => { this.lastUserInteraction = performance.now(); if (this.autoDriver) this.autoDriver.forceStop(); }; this.autoDriver = new AutoDriver(Mouse, this, { enabled: props.autoDemo, speed: props.autoSpeed, resumeDelay: props.autoResumeDelay, rampDuration: props.autoRampDuration }); this.init(); this._loop = this.loop.bind(this); this._resize = this.resize.bind(this); window.addEventListener('resize', this._resize); this._onVisibility = () => { const hidden = document.hidden; if (hidden) { this.pause(); } else if (isVisibleRef.current) { this.start(); } }; document.addEventListener('visibilitychange', this._onVisibility); this.running = false; }
      init() { this.props.$wrapper.prepend((Common.renderer as THREE.WebGLRenderer).domElement); this.output = new Output(); }
      ...
      dispose() { try { window.removeEventListener('resize', this._resize); document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); if (Common.renderer) { const canvas = (Common.renderer as THREE.WebGLRenderer).domElement; if (canvas && canvas.parentNode) canvas.parentNode.removeChild(canvas); (Common.renderer as THREE.WebGLRenderer).dispose(); } } catch (_) {} }
    }
